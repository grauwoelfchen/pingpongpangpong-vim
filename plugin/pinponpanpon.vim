...
function! s:PinponPanpon(...)
  ...
  if !(empty(name) || empty(code))
    ...
    let response = webapi#http#get(base_url.code.'_table.html')
    " jma.go.jp has broken table ;(
    let replaced = substitute(response.content, "</tr></tr>", "</tr>", 'g')
    let document = webapi#html#parse(iconv(replaced, 'utf-8', &encoding))
    redraw | echo 'parsing data...'
    for a in document.findAll('table', {'id':'WarnTableTable'})[0].findAll('a')
      if has_key(a.attr, 'href')
        if a.value() == name
          redraw | echo 'fetching info...'
          let attr_hrf = a.attr['href']
          let response = webapi#http#get(base_url.attr_hrf)
          let document = webapi#html#parse(iconv(response.content, 'utf-8', &encoding))
          redraw | echo 'parsing info...'
          for td in document.findAll('table', {'id':'WarnInfoTable'})[0].findAll('td')
            if !empty(td.value())
              redraw | echo ''
